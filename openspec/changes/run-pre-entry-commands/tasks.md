...
- [ ] 1.1 Add strict `sandbox.hooks.initialize` command objects to the
      ...
      fields.
- [ ] 1.2 Carry defensive copies of validated command vectors through the domain
...
- [ ] 2.1 Add a narrow command-execution request and capability to the Sandbox
      port for sandbox name, workspace, argument vector, and attached output
      streams.
- [ ] 2.2 Implement Docker Sandbox exec invocation in the outbound adapter, with
...
- [ ] 3.1 Add initialization to `up` after creation and network-policy
...
- [ ] 3.2 Add lifecycle tests for ordered execution, explicit-shell and literal
...
- [ ] 3.3 Add executable-level CLI coverage proving malformed hooks fail before
...
- [ ] 4.1 Document `sandbox.hooks.initialize`, creation-only non-interactive
...
- [ ] 4.2 Update the `personal-site` example with bounded `language-node-npm`
      mount readiness plus repository bootstrap commands, keeping the example
      valid against the published schema.
- [ ] 4.3 Format the repository and run focused declaration, configuration,
