...
- [ ] 1.1 Add a destroy lifecycle runner that reuses target resolution and
...
- [ ] 1.2 Add focused lifecycle tests for resolution and executable failures,
...
- [ ] 2.1 Add an injected Cobra `destroy` command with no positional arguments,
      `--force`/`-f`, repository working-directory discovery, all three stream
      bindings, contextual help, and attached-error suppression.
- [ ] 2.2 Register the destroy dependency in root construction and update CLI
...
- [ ] 2.3 Add executable-level coverage proving target-only resolution,
...
- [ ] 3.1 Update README availability, command usage, confirmation and force
...
- [ ] 4.1 Run `mise run fmt` and inspect the resulting Go, Markdown, and OpenSpec
      artifact changes.
- [ ] 4.2 Run focused Go tests for lifecycle, CLI, and executable behavior, then
