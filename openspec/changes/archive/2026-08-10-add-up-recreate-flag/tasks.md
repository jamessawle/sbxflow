...
- [x] 1.1 Add a lifecycle-owned up options value carrying the recreation choice,
...
- [x] 1.2 Implement the validated lookup branches so an absent sandbox follows
...
- [x] 1.3 Add lifecycle tests for ordinary behavior, absent-sandbox recreation,
...
- [x] 2.1 Register the long-form-only `up --recreate` flag, describe its
      ...
      error rendering.
- [x] 2.2 Update CLI tests for help discovery, default and recreate option
...
- [x] 2.3 Extend the entrypoint's narrowed up dependency with the Sandbox remover
      capability and update root or executable-level tests for the changed runner
      contract and advertised interface.

## 3. Documentation

- [x] 3.1 Update README `up` usage and lifecycle guidance to explain normal
...
- [x] 4.1 Run `mise run fmt` and review the resulting Markdown and Go formatting.
- [x] 4.2 Run `mise run validate` to verify strict OpenSpec checks, architecture
