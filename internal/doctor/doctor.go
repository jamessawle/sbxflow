...
func NewRunner(commands CommandRunner, checks ...Check) Runner {
	return Runner{commands: commands, checks: checks}
}
...
func (r Runner) Run(ctx context.Context) Report {
	...
	for _, check := range r.checks {
		...
		result := check.Run(ctx, Environment{Commands: r.commands, Facts: facts})
		result.ID = check.ID()
