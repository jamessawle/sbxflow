...
func NewDefaultRunner() Runner {
	commands := ExecCommandRunner{Timeout: defaultCommandTimeout}
	return NewRunner(
		commands,
		compatibilityCheck{},
		diagnosticsCheck{},
...
func (compatibilityCheck) Run(ctx context.Context, environment Environment) Result {
	executable, err := environment.Commands.LookPath("sbx")
	if err != nil {
		return Result{
			Status:   StatusFail,
	...
	output := environment.Commands.Run(ctx, executable, "version")
	if output.Err != nil {
		return Result{
			Status:   StatusFail,
...
func (diagnosticsCheck) Run(ctx context.Context, environment Environment) Result {
	output := environment.Commands.Run(
		ctx,
		environment.Facts[FactSbxExecutable],
